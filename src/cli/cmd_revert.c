...
/* Thin argument-parsing shell over the shared replay engine in
   src/cli/pick.c (see include/sg/pick.h) -- kept byte-for-byte parallel to
   cmd_cherry_pick.c's shell on purpose, the two commands share one engine
   parameterised by sg_seq_kind. Do not let the two shells drift apart
   without a reason; a divergence here is either a genuine flag difference
   (there is none between cherry-pick and revert in this phase's scope) or
   a bug. */

static const char usage[] =
    "usage: sg revert [-n|--no-commit] [-m <parent-number>] [--no-edit] <commit>...\n"
    "       sg revert (--continue | --skip | --abort | --quit)\n";
...
int sg_cmd_revert(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--continue") == 0) {
            ...
            /* Every deliberately-unimplemented flag from the Phase 57 spec
               (A..B ranges, -e/--edit, -x, -s/--signoff, -S, --ff,
               --allow-empty, --allow-empty-message,
               --keep-redundant-commits, --empty=, --cleanup=, --strategy,
               -X, --rerere-autoupdate, --reference, and anything else
               unrecognized) falls through to here: rejected with the usage
               line, never approximated. */
            fputs(usage, stderr);
            free(commit_args);
            return 1;
        } else {
    ...
    if (continue_flag) {
        ...
        rc = sg_pick_skip(git_dir, repo_root, SG_SEQ_REVERT);
    } else if (abort_flag) {
        rc = sg_pick_abort(git_dir, repo_root, SG_SEQ_REVERT);
    } else if (quit_flag) {
        rc = sg_pick_quit(git_dir, repo_root, SG_SEQ_REVERT);
    } else {
        ...
        for (i = 0; i < commit_count; i++) {
            ...
            if (prc != 0) {
                ...
                if (prc == -4) {
                    ...
                    sg_cli_report_rev_error("revert", SG_REV_ERR_NOT_A_REV, commit_args[i], NULL, 0);
                } else {
                    sg_cli_report_rev_error("revert",
        ...
        rc = sg_pick_start(git_dir, repo_root, SG_SEQ_REVERT, ids, (size_t)commit_count, &opts);
        free(ids);
    }
